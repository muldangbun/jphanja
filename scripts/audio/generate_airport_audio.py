...
JSON_FILE = "../../data/scenes/airport_scenesv1.JSON"
...
def main():
    ...
    for scenario in data["scenarios"]:
        ...
        for i, item in enumerate(scenario["dialogue"]):
            ...
            # 캐릭터별 목소리 선택
            # 점원(공항 직원/승무원): 8 (카스가베 츠무기 - 차분하고 친절한 여성 톤)
            # 손님(여행객): 10 (아오야마 류세이 - 일반 남성 톤)
            # 심사관 등 혹시 모를 다른 이름이 있을 경우를 위해 기본값 설정
            if "손님" in speaker or "여행객" in speaker:
                speaker_id = 10
            elif "점원" in speaker or "직원" in speaker or "승무원" in speaker or "심사관" in speaker:
                speaker_id = 8
            else:
